...
bool W25qxx_Init(void)
{
	...
	W25qxx_Delay(100);
	uint32_t id;
	DEBUG_W25QXX("w25qxx Init Begin...\r\n");
	id = W25qxx_ReadID();
	DEBUG_W25QXX("w25qxx ID:0x%X\r\n", id);
	switch (id & 0x000000FF)
	{
	case 0x20: // 	w25q512
		w25qxx.ID = W25Q512;
		w25qxx.BlockCount = 1024;
		DEBUG_W25QXX("w25qxx Chip: w25q512\r\n");
		break;
	case 0x19: // 	w25q256
		w25qxx.ID = W25Q256;
		w25qxx.BlockCount = 512;
		DEBUG_W25QXX("w25qxx Chip: w25q256\r\n");
		break;
	case 0x18: // 	w25q128
		w25qxx.ID = W25Q128;
		w25qxx.BlockCount = 256;
		DEBUG_W25QXX("w25qxx Chip: w25q128\r\n");
		break;
	case 0x17: //	w25q64
		w25qxx.ID = W25Q64;
		w25qxx.BlockCount = 128;
		DEBUG_W25QXX("w25qxx Chip: w25q64\r\n");
		break;
	case 0x16: //	w25q32
		w25qxx.ID = W25Q32;
		w25qxx.BlockCount = 64;
		DEBUG_W25QXX("w25qxx Chip: w25q32\r\n");
		break;
	case 0x15: //	w25q16
		w25qxx.ID = W25Q16;
		w25qxx.BlockCount = 32;
		DEBUG_W25QXX("w25qxx Chip: w25q16\r\n");
		break;
	case 0x14: //	w25q80
		w25qxx.ID = W25Q80;
		w25qxx.BlockCount = 16;
		DEBUG_W25QXX("w25qxx Chip: w25q80\r\n");
		break;
	case 0x13: //	w25q40
		w25qxx.ID = W25Q40;
		w25qxx.BlockCount = 8;
		DEBUG_W25QXX("w25qxx Chip: w25q40\r\n");
		break;
	case 0x12: //	w25q20
		w25qxx.ID = W25Q20;
		w25qxx.BlockCount = 4;
		DEBUG_W25QXX("w25qxx Chip: w25q20\r\n");
		break;
	case 0x11: //	w25q10
		w25qxx.ID = W25Q10;
		w25qxx.BlockCount = 2;
		DEBUG_W25QXX("w25qxx Chip: w25q10\r\n");
		break;
	default:
		DEBUG_W25QXX("w25qxx Unknown ID\r\n");
		w25qxx.Lock = 0;
		return false;
	}
	w25qxx.PageSize = 256;
	...
	DEBUG_W25QXX("w25qxx Page Size: %d Bytes\r\n", w25qxx.PageSize);
	DEBUG_W25QXX("w25qxx Page Count: %d\r\n", w25qxx.PageCount);
	DEBUG_W25QXX("w25qxx Sector Size: %d Bytes\r\n", w25qxx.SectorSize);
	DEBUG_W25QXX("w25qxx Sector Count: %d\r\n", w25qxx.SectorCount);
	DEBUG_W25QXX("w25qxx Block Size: %d Bytes\r\n", w25qxx.BlockSize);
	DEBUG_W25QXX("w25qxx Block Count: %d\r\n", w25qxx.BlockCount);
	DEBUG_W25QXX("w25qxx Capacity: %d KiloBytes\r\n", w25qxx.CapacityInKiloByte);
	DEBUG_W25QXX("w25qxx Init Done\r\n");
	w25qxx.Lock = 0;
	return true;
}
//###################################################################################################################
void W25qxx_EraseChip(void)
{
	...
	DEBUG_W25QXX("w25qxx EraseChip Begin...\r\n");
	W25qxx_WriteEnable();
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	W25qxx_Spi(0xC7);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
	W25qxx_WaitForWriteEnd();
	DEBUG_W25QXX("w25qxx EraseBlock done after %d ms!\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(10);
	w25qxx.Lock = 0;
}
//###################################################################################################################
void W25qxx_EraseSector(uint32_t SectorAddr)
{
	...
	w25qxx.Lock = 1;
	uint32_t StartTime = HAL_GetTick();
	DEBUG_W25QXX("w25qxx EraseSector %d Begin...\r\n", SectorAddr);
	W25qxx_WaitForWriteEnd();
	...
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
	W25qxx_WaitForWriteEnd();
	DEBUG_W25QXX("w25qxx EraseSector done after %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(1);
	w25qxx.Lock = 0;
}
//###################################################################################################################
void W25qxx_EraseBlock(uint32_t BlockAddr)
{
	while (w25qxx.Lock == 1)
		W25qxx_Delay(1);
	w25qxx.Lock = 1;
	DEBUG_W25QXX("w25qxx EraseBlock %d Begin...\r\n", BlockAddr);
	W25qxx_Delay(100);
	uint32_t StartTime = HAL_GetTick();
	W25qxx_WaitForWriteEnd();
...
bool W25qxx_IsEmptyPage(uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToCheck_up_to_PageSize)
{
	...
	DEBUG_W25QXX("w25qxx CheckPage is Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return true;
NOT_EMPTY:
    DEBUG_W25QXX("w25qxx CheckPage is Not Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return false;
}
...
bool W25qxx_IsEmptySector(uint32_t Sector_Address, uint32_t OffsetInByte, uint32_t NumByteToCheck_up_to_SectorSize)
{
	...
	DEBUG_W25QXX("w25qxx CheckSector is Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return true;
NOT_EMPTY:
	DEBUG_W25QXX("w25qxx CheckSector is Not Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return false;
}
...
bool W25qxx_IsEmptyBlock(uint32_t Block_Address, uint32_t OffsetInByte, uint32_t NumByteToCheck_up_to_BlockSize)
{
	...
	DEBUG_W25QXX("w25qxx CheckBlock is Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return true;
NOT_EMPTY:
	DEBUG_W25QXX("w25qxx CheckBlock is Not Empty in %d ms\r\n", HAL_GetTick() - StartTime);
	W25qxx_Delay(100);
	w25qxx.Lock = 0;
	return false;
}
...
void W25qxx_WriteByte(uint8_t pBuffer, uint32_t WriteAddr_inBytes)
{
	...
	w25qxx.Lock = 1;
	uint32_t StartTime = HAL_GetTick();
	DEBUG_W25QXX("w25qxx WriteByte 0x%02X at address %d begin...", pBuffer, WriteAddr_inBytes);
	W25qxx_WaitForWriteEnd();
	W25qxx_WriteEnable();
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	...
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
	W25qxx_WaitForWriteEnd();
	DEBUG_W25QXX("w25qxx WriteByte done after %d ms\r\n", HAL_GetTick() - StartTime);
	w25qxx.Lock = 0;
}
//###################################################################################################################
...
void W25qxx_WriteBlock(uint8_t *pBuffer, uint32_t Block_Address, uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_BlockSize)
{
	...
	DEBUG_W25QXX("+++w25qxx WriteBlock:%d, Offset:%d ,Write %d Bytes, begin...\r\n", Block_Address, OffsetInByte, NumByteToWrite_up_to_BlockSize);
	W25qxx_Delay(100);
	if (OffsetInByte >= w25qxx.BlockSize)
	{
		DEBUG_W25QXX("---w25qxx WriteBlock Faild!\r\n");
		W25qxx_Delay(100);
		return;
	}
	uint32_t StartPage;
	...
	DEBUG_W25QXX("---w25qxx WriteBlock Done\r\n");
	W25qxx_Delay(100);
}
//###################################################################################################################
void W25qxx_ReadByte(uint8_t *pBuffer, uint32_t Bytes_Address)
{
	...
	w25qxx.Lock = 1;
	uint32_t StartTime = HAL_GetTick();
	DEBUG_W25QXX("w25qxx ReadByte at address %d begin...\r\n", Bytes_Address);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
...
void W25qxx_ReadBytes(uint8_t *pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
	...
	w25qxx.Lock = 1;
	uint32_t StartTime = HAL_GetTick();
	DEBUG_W25QXX("w25qxx ReadBytes at Address:%d, %d Bytes  begin...\r\n", ReadAddr, NumByteToRead);
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
	...
	DEBUG_W25QXX("\r\n");
	DEBUG_W25QXX("w25qxx ReadBytes done after %d ms\r\n", StartTime);
	W25qxx_Delay(100);
	W25qxx_Delay(1);
	w25qxx.Lock = 0;
}
...
void W25qxx_ReadSector(uint8_t *pBuffer, uint32_t Sector_Address, uint32_t OffsetInByte, uint32_t NumByteToRead_up_to_SectorSize)
{
	...
	DEBUG_W25QXX("+++w25qxx ReadSector:%d, Offset:%d ,Read %d Bytes, begin...\r\n", Sector_Address, OffsetInByte, NumByteToRead_up_to_SectorSize);
	W25qxx_Delay(100);
	if (OffsetInByte >= w25qxx.SectorSize)
	{
		DEBUG_W25QXX("---w25qxx ReadSector Faild!\r\n");
		W25qxx_Delay(100);
		return;
	}
	uint32_t StartPage;
	...
	DEBUG_W25QXX("---w25qxx ReadSector Done\r\n");
	W25qxx_Delay(100);
}
//###################################################################################################################
void W25qxx_ReadBlock(uint8_t *pBuffer, uint32_t Block_Address, uint32_t OffsetInByte, uint32_t NumByteToRead_up_to_BlockSize)
{
	...
	DEBUG_W25QXX("+++w25qxx ReadBlock:%d, Offset:%d ,Read %d Bytes, begin...\r\n", Block_Address, OffsetInByte, NumByteToRead_up_to_BlockSize);
	W25qxx_Delay(100);
	if (OffsetInByte >= w25qxx.BlockSize)
	{
		DEBUG_W25QXX("w25qxx ReadBlock Faild!\r\n");
		W25qxx_Delay(100);
		return;
	}
	uint32_t StartPage;
	...
	DEBUG_W25QXX("---w25qxx ReadBlock Done\r\n");
	W25qxx_Delay(100);
}
//###################################################################################################################
